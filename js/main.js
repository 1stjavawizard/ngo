...
var onloadCallback = function() {
    grecaptcha.render('google_recaptcha', { 'sitekey': '6Lf10qoUAAAAAJgpJKRFM815Me_Iow0_dHbaSXly' });
};
*/

$(document).on('click', '.donate_it', function() {
	...
	//	let amount2 =(amount.indexOf(',') != -1)? amount : parseInt(amount.split(','));
		
		 let donation_details ={'amount':amount,'email':email,'phone':phone};


       
           // var json = JSON.parse(JSON.stringify(user_data));

            var handler = PaystackPop.setup({
                key: 'pk_test_c35dad57e3d08ac46de32d2bca126ae7f235051f',
                email: email,
                amount: amount + '00',
                currency: 'NGN',
                ...
                metadata: {
                    custom_fields: [{
                            display_name: "Donation infomation",
                            variable_name: "Donation",
                            value: "Donation to  orphange house"
                        }
                    ]
                },
                callback: function(response) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "database/server?action=pay",
                        ...
                        success: function(data) {
                           ...
                           /*
                           Swal.fire({
                              icon: '<span color:black>success </span>',
                              title: '<p style="color:black;text-align:center">Thank you</p>',
                              text: 'A thank you message has been sent to you!',
                              
                            }).then(function(){
                                window.location.href = "https://mentscrm.com.ng/liftfund/";
                           });*/
...
$(document).on('click', '.donate_it4', function() {
        let donation = $(this).serialize();
 ...
 function makePayment() {
    FlutterwaveCheckout({
      public_key: "FLWPUBK-1038da40fcdfdc2104f718f406f6ece1-X",
      ...
      callback: function (data) {
        $.ajax({
                        ...
                        success: function(data) {
                           ...
                           Swal.fire('Thank You! Message has been sent to your email');
                            window.location.href = "https://mentscrm.com.ng/liftfund/";
                             // window.location.href = "https://mentscrm.com.ng/liftfund/";
      ...
      customizations: {
        title: "My Donations",
        description: "Donation for divine destiny orphange home",
...
$(document).on('submit', '#active-contact-form',function(){
     let contactus = $(this).serialize();
    
      $.ajax({
                        type: "POST",
                        url: "database/server?action=contactus",
                        data: contactus,
                        dataType: "json",
                        success: function(data) {
                            if (data !=='0') {
...
$(document).on('click', '#sublettermail',function(){
    	let newslettermail = $('#newslettermail').val();
    	let details = {'email':newslettermail};
    	alert(newslettermail);
      $.ajax({
                        type: "POST",
                        url: "database/server?action=subscribe",
                        data: details,
                        dataType: "json",
                        success: function(data) {
                            if (data !=='0') {
...
$(document).on('click', '#signin',function(){
     let contactus = $(this).serialize();
      ...
      $.ajax({
                        type: "POST",
                        url: "database/server?action=signin",
                        data: contactus,
                        dataType: "json",
                        success: function(data) {
                            if (data !=='0') {
                               //  Swal.fire('Your Message has been sent');
                               // alert("message is sent");
                                window.location.href = "https://mentscrm.com.ng/liftfund/dashboard";
                                  } 
...
/*  ======================================
    ...
    if ($('#map').length > 0){
        var myCenter = new google.maps.LatLng(6.589692,3.268698);
        function initialize() {
            ...
            var marker = new google.maps.Marker({
                position : new google.maps.LatLng(6.589692,3.268698),
                map : map
            });
        }
